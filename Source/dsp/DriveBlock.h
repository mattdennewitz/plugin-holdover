...
private:
    ...
    // Class-A asymmetry -> 2nd-harmonic growl. biasNominal_ is the per-channel setpoint from
    // setCharacter (a shared in-phase base + a small opposite-sign trim, so the even content
    // survives a mono fold while still de-correlating L/R). processSample scales it by drive
    // (asymmetry tracks how hard the stage is pushed) and one-pole smooths bias_ toward it so
    // sweeping Character doesn't zipper through the shaper + DC blocker.
    float biasNominal_ = 0.0f, bias_ = 0.0f, biasCoeff_ = 0.0f;
    Shelf pre_, de_;
